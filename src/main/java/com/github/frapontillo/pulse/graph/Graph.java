...
import com.github.frapontillo.pulse.spi.ProcessInfo;

import java.util.*;
import java.util.stream.Collectors;

/**
 * Directed graph implementation to load Pulse configurations from.
 *
...
public class Graph {
    ...
    /**
     * Get the list of edges in the Graph as a {@link HashMap}, where the key is the name of the
     * {@link Node} where the edge starts from and the value is a list of {@link Node} names where
     * the edge ends.
     *
    ...
    /**
     * Set the list of edges in the Graph as a {@link HashMap}, where the key is the name of the
     * {@link Node} where the edge starts from and the value is a list of {@link Node} names where
     * the edge ends.
     *
    ...
    /**
     * Update the {@link List} of {@link Node}s that aren't referenced by any other node in the
     * directed Graph.
     *
    ...
    protected List<Node> updateRootNodes() {
        ...
        roots.addAll(nodes.keySet().stream().filter(nodeKey -> !referencesNodes.contains(nodeKey))
    ...
    protected List<Node> updateTerminalNodes() {
        ...
        terminals.addAll(nodes.keySet().stream().filter(nodeKey -> !fatherNodes.contains(nodeKey))
                              .map(nodes::get).collect(Collectors.toList()));
        return terminals;
    }
    ...
    /**
     * Recursively build a {@link Node} children in the Graph, one by one, by checking if they
     * weren't already built.
     *
    ...
    public Graph appendSingleTerminal(Node newTerminal) {
        ...
        // ad the new edges
        terminals.forEach(
                t -> edges.put(t.getName(), Collections.singletonList(newTerminal.getName())));
